...
class WindResourceDatasetStatistics(EyaDefBaseModel):
    ...
    data_availability: list[Dataset] = pdt.Field(
        ...
        description=(
            ...
            "basis, with the dimensions 'wind_dataset_id', 'point_id', "
            "'year' and 'month' (in that order) for datasets from wind "
            "measurement stations and meteorological reference "
            "sources; and 'wind_farm_id', 'operational_dataset_id', "
            ...
            "identical across all turbines, otherwise omitted), 'year' "
            "and 'month' (in that order) for operational reference "
            "wind farm datasets. Additional datasets with other "
            "dimensions may be included optionally. The raw data "
...
class WindResourceResults(EyaDefBaseModel):
    ...
    wind_speed: list[Dataset] = pdt.Field(
        ...
        description=(
            ...
            "'wind_dataset_id' and 'height' (in that order). Further "
    ...
    probability: list[Dataset] = pdt.Field(
        ...
        description=(
            ...
            "dimensions 'wind_dataset_id', 'height', 'wind_speed' and "
            "'wind_from_direction' (in that order). The wind speed "
    ...
    wind_shear_exponent: Optional[list[Dataset]] = pdt.Field(
        ...
        description=(
            ...
            "dataset should be 'wind_dataset_id'. Further results with "
    ...
    air_density: Optional[list[Dataset]] = pdt.Field(
        ...
        description=(
            ...
            "result dataset should be 'wind_dataset_id' and 'height' "
            "(in that order). Further results with other dimensions "
    ...
    displacement_height: Optional[list[Dataset]] = pdt.Field(
        ...
        description=(
            ...
            "'wind_dataset_id'. Further results with other dimensions, "
            "such as displacement height binned by wind direction, may "
            "also be included."
        ),
    )
...
class TurbineWindResourceWeighting(EyaDefBaseModel):
    ...
    source_wind_data: Optional[list[Dataset]] = pdt.Field(
        ...
        description=(
            ...
            "prediction the weight is applied to), and 'variable_id' "
            "(if different weights were applied to different "
            "variables, such as wind speed and wind direction weighted "
            "differently, otherwise omitted). Further results with "
            "additional dimensions may optionally be included, in "
            "which case the initial set of dimensions must be the same "
            "as those for the first standard dataset. For example, if "
            "weights are applied by wind direction bin, an additional "
            "dataset with dimensions 'turbine_id', 'wind_dataset_id', "
            "'variable_id' and 'wind_from_direction' (i.e. wind "
            "direction bin) may be included."
        ),
    )
